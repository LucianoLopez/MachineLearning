...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.graphDic[node][INPUT]
    ...
    def get_output(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.graphDic[node][OUTPUT]
    ...
    def get_gradient(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.graphDic[node][GRADIENT]
    ...
    def add(self, node):
        ...
        attributes[GRADIENT] = np.zeros_like(output)
    ...
    def backprop(self):
        ...
        "*** YOUR CODE HERE ***"
        self.graphDic[loss_node][GRADIENT] = 1.0
        gradients = loss_node.backward(self.graphDic[loss_node][INPUT], 1.0)
        parents = loss_node.get_parents()
        for i in range(len(parents)):
            self.graphDic[parents[i]][GRADIENT] += gradients[i]
        copyNodes = self.get_nodes()[::-1][1:]
        for node in copyNodes:
            nodeDic = self.graphDic[node]
            gradients = node.backward(nodeDic[INPUT], nodeDic[GRADIENT])
            parentals = node.get_parents()
            for y in range(len(parentals)):
                self.graphDic[parentals[y]][GRADIENT] += gradients[y]
    ...
    def step(self, step_size):
        ...
        for variable in self.variables:
            variable.data = variable.data - (self.graphDic[variable][GRADIENT] * step_size)
...
class Add(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        matrix = inputs[0] + inputs[1]
        return matrix

    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        lst = [gradient, gradient]
        return lst
...
class MatrixMultiply(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        matrix = np.dot(inputs[0], inputs[1])
        return matrix

    @staticmethod
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        derivA = np.dot(gradient, inputs[1].T)
        derivB = np.dot(inputs[0].T, gradient)
        lst = [derivA, derivB]
        return lst
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        lst = [gradient, np.sum(gradient, 0)]
        return lst
...
class ReLU(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        matrix = np.zeros_like(inputs[0])
        coord = np.where(inputs[0] > 0)
        matrix[coord] = inputs[0][coord]
        return matrix
    ...
    def backward(inputs, gradient):
        ...
        derivMatrix = np.maximum(inputs[0], zeroMatrix)
        coord = np.where(derivMatrix > 0)
        derivMatrix[coord] = 1
        derivMatrix = gradient * derivMatrix
        return [derivMatrix]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        derivA = (inputs[0] - inputs[1]) / inputs[0].size
        derivB = (inputs[1] - inputs[0]) / inputs[0].size
        derivA = np.dot(gradient, derivA)
        derivB = np.dot(gradient, derivB)
